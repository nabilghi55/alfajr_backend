...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        @foreach ($pakets as $item)
            <div onclick="openModal({
                ...
                deskripsi: `{!! nl2br(e($item->itinerary ?? '-')) !!}`,
                gambar: '{{ asset('storage/' . $item->gambar_unggulan) }}'
            })"
                class="bg-white shadow-lg rounded-2xl overflow-hidden hover:shadow-xl transition duration-300 cursor-pointer">

                <!-- Gambar -->
                @if ($item->gambar_unggulan)
                    <img src="{{ asset('storage/' . $item->gambar_unggulan) }}" class="w-full h-48 object-cover">
                @else
    ...
    <div id="modalDetail" class="fixed inset-0 bg-black/60 backdrop-blur-sm hidden justify-center items-center z-50">
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-xl overflow-hidden animate-fadeIn">
    ...
    <script>
        function openModal(data) {
            ...
            document.getElementById("modalHarga").innerText = "Rp " + data.harga.toLocaleString();
